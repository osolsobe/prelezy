...
export default async function SlotPage({
  ...
  const route = slot.assignments[0]?.route ?? null;
  const cs = locale === "cs";
    ...
    return (
      <div className="max-w-lg mx-auto text-center py-16">
        ...
        <p className="text-gray-500">
          {cs ? "Na tomto slotu není aktuálně žádná cesta." : "No route is currently assigned to this slot."}
        </p>
      </div>
    );
  }
  ...
  return (
    <div className="max-w-lg mx-auto">
      ...
      <div className="grid grid-cols-3 gap-3 mb-4">
        <StatCard label={cs ? "Pokusů" : "Attempts"} value={route._count.attempts} />
        <StatCard label={cs ? "Přelezů" : "Ascents"} value={successCount} />
        <StatCard label={cs ? "Úspěšnost" : "Success"} value={`${successRate}%`} />
      </div>
